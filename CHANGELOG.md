...
- 配置 `toolIsolation`: none | session-subdir | session-lock（默认 none）
- 模块 `src/harness/tool-effect/isolation.ts`
## v0.37.0 (2026-09-26)

### feat(harness): Phase C Session 一等数据 — `primaryAgentId` + 消息归因

- 双键 store 保持；`SessionData.primaryAgentId?` create 写入 + 历史回填
- `Message.agentId?` assistant/tool 写回归因
- Jsonl 持久化 primaryAgentId

## v0.38.0 (2026-09-26)

### feat(harness): Phase D Compact 互斥与键位 — E4 `(sessionId, agentId)`

- compact 与 handle **共 session 锁**；Gateway 不再仅靠 status
- 键：`contextCompacts[agentId]` + 引擎/Agent Map `sessionId::agentId`

## v0.39.0 (2026-09-26)

### feat(harness): Phase E Session ACL / 角色目录 — E6 最小集

- 出厂五角色 owner/specialist/reviewer/operator/steward
- `grant`/`revoke`/`authorizeRun`；effective = L0 ∩ role.max ∩ agent.max ∩ 绑定
- 配置 `sessionAcl`；**无角色 DB**

## v0.41.0 (2026-09-26)

### feat(harness) + docs: Phase G 预留位硬化

- `SessionLease` 接口 + `InProcessSessionLock`（E2/E7）
- `agentRevision?` 字段位；Quota 挂载点文档预留
## v0.35.6 (2026-09-26)
