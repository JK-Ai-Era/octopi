...
> **概念约定**：产品**八层** = system 侧 ContextLayer 1–7（含 **Runtime** 为第 7 层）+ **Information**（第 8 层，消息窗口 strip）。`ContextLayerId` 仍为 7 个 system id，不为八层增删。所有权与 Scope 见 [架构宪法](./north-star.md) §1.3。
