...
import { classifyError as classifyErrorNew } from './loop/error-classifier.js';
...
export class AgentEngine {
  ...
  private classifyError(err: unknown): ClassifiedError {
    const result = classifyErrorNew(err);
    return { reason: result.reason, message: result.message, originalError: result.originalError, retryAfterMs: result.retryAfterMs };
  }
