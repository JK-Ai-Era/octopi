/**
 * MessageChannel — 进程间/节点间通信协议
 *
 * 职责：定义 Agent 之间如何点对点通信（消息语义）。
 * 实现方：内存队列（默认可嵌入）、Worker、Unix Socket、WebSocket、消息队列等。
 *
 * 设计要点：
 * - 点对点消息传递（有明确的接收方）
 ...
 * - 消息有序、不丢失（由实现方保证）
 *
 * 分层：Core 只保留本契约。进程内默认实现与未来的 worker/network
 * transport 放在 Integration；multi-agent 依赖语义，不绑定 OS IPC。
 */
 ...
 * MessageChannel 接口
 *
 * 提供点对点消息传递能力。Core 不绑定具体 transport；
 * Harness/Integration 按部署形态注入实现。
 */
